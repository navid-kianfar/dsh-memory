 ...
 *
 * The dialog is a scrolling card inside the design system's modal, so a suggestion list rendered in
 * place is clipped before it reaches its second row. Portalling escapes that; the cost is that
 * position has to be computed rather than inherited, which is what this module is.
 *
 * The design system ships `Menu`, and every plain picker here uses it. This exists for the surface
 * `Menu` cannot be: a list that belongs to a text box still holding the caret.
 *
 * @module @achasoft/dsh-memory/client/ui/Popover
 */
...
export function Popover({ open, anchorRef, onClose, align = 'start', label, className, children }: {
  ...
  useEffect(() => {
    ...
    // Capture: the screen's own panes scroll, and a scroll event on a nested pane does not bubble.
  ...
  useEffect(() => {
     ...
     *
     * Stopped here rather than let through: the surface sits inside a dialog, and one Escape must
     * close the list the caret is in rather than the whole form around it.
    ...
    /**
     * Close on a press outside both the surface and its anchor.
     *
     * The anchor is excluded because it toggles: closing here as well would close and reopen in one
     * press, which reads as the list refusing to open.
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-label={label}
      className={cx(fields.fields, fields.popover, css.surface, className)}
