 ...
 *    registry. `@duckdb/node-api` stays external for a different reason: it loads a per-platform
 *    native binding, which no bundler can inline.
...
/**
 * Specifiers the Node half must never inline: the harness's own packages (shared singletons) and the
 * DuckDB driver (a native addon).
 * @param specifier - the import as written.
 * @returns true when the import must survive into the emitted bundle.
 */
function isExternalOnNode(specifier: string): boolean {
  return specifier.startsWith('@deepseek-ai/')
    || specifier === 'zod'
    || specifier === '@duckdb/node-api'
    || specifier.startsWith('@duckdb/')
}
 ...
 * @param file - absolute path of the stylesheet, used to name its style tag.
 * @param css - the compiled, minified CSS.
...
const nodeHalf: UserConfig = {
  ...
  entry: {
    ...
    tools: `${TSC_OUT}/tools/index.js`,
    'embeddings-openai': `${TSC_OUT}/providers/openai-embeddings.js`,
  ...
  clean: true,
  deps: {
    neverBundle: isExternalOnNode,
    alwaysBundle: (specifier: string) => !isBuiltin(specifier) && !isExternalOnNode(specifier),
  },
}
